...
"""
    createcache(sp, dtype::UserDeriv, func!, nx, ng)

Cache for dense or sparse jacobian with user-supplied derivatives
...
- `ng::Int`: number of constraints
"""
function createcache(sp, dtype::UserDeriv, func!, nx, ng)
    # Jwork = zeros(ng, nx)
    return DenseCache(func!, 0.0, 0.0, nothing, dtype)
end
